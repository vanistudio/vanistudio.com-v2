...
interface QuickReorderApiGroupsDialogProps {
  open: boolean;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="size-6 rounded flex items-center justify-center shrink-0 border bg-background border-border text-vanixjnk">
          <Icon icon="solar:folder-2-line-duotone" className="size-4" />
        </div>
        <div className="flex flex-col min-w-0">
          <span className="text-[13px] font-bold text-foreground truncate">{name}</span>
          <span className="text-[10px] font-mono text-muted-foreground truncate">{slug}</span>
        </div>
      </div>
...
export function QuickReorderApiGroupsDialog({
  open,
  onOpenChange,
  apiType,
  onSuccess,
}: QuickReorderApiGroupsDialogProps) {
  const [items, setItems] = useState<any[]>([]);
  const utils = trpc.useUtils();
  const groupsQuery = trpc.administrator.apiDocs.getGroupsWithEndpoints.useQuery(
    { apiType },
    { enabled: open && !!apiType }
  );
  ...
  }, [open, groupsQuery.data]);

  const reorderMutation = trpc.administrator.apiDocs.reorderGroups.useMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Đã cập nhật thứ tự nhóm API!");
      utils.administrator.apiDocs.getGroupsWithEndpoints.invalidate();
      ...
      <DialogContent className="sm:max-w-[550px] w-[95vw]">
        <DialogHeader>
          ...
          <DialogDescription className="text-left mt-1 text-[13px]">
            Kéo thả để sắp xếp thứ tự hiển thị của các Nhóm API.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="overflow-y-auto min-h-[150px] max-h-[45vh] custom-scrollbar pr-1 flex flex-col gap-2">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center text-muted-foreground">
                ...
                <span className="text-[13px]">Chưa có nhóm API nào.</span>
              </div>
            ) : (

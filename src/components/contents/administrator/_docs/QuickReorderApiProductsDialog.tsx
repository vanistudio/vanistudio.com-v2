...
interface QuickReorderApiProductsDialogProps {
  open: boolean;
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="size-6 rounded flex items-center justify-center shrink-0 border bg-background border-border text-vanixjnk">
          <Icon icon="solar:box-minimalistic-line-duotone" className="size-4" />
        </div>
...
export function QuickReorderApiProductsDialog({
  open,
  onOpenChange,
  onSuccess,
}: QuickReorderApiProductsDialogProps) {
  const [items, setItems] = useState<any[]>([]);
  const utils = trpc.useUtils();
  const productsQuery = trpc.administrator.apiDocs.getApiProducts.useQuery(
    undefined,
    { enabled: open }
  );
  ...
  const reorderMutation = trpc.administrator.apiDocs.reorderApiProducts.useMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Đã cập nhật thứ tự sản phẩm/API!");
      ...
      <DialogContent className="sm:max-w-[550px] w-[95vw]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            Sắp xếp sản phẩm/API
          </DialogTitle>
          <DialogDescription className="text-left mt-1 text-[13px]">
            Kéo thả để sắp xếp thứ tự hiển thị của các sản phẩm/API trên trang tài liệu.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="overflow-y-auto min-h-[150px] max-h-[45vh] custom-scrollbar pr-1 flex flex-col gap-2">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center text-muted-foreground">
                ...
                <span className="text-[13px]">Chưa có sản phẩm/API nào.</span>
              </div>
            ) : (

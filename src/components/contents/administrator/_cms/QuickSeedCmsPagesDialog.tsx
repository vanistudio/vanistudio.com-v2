...
interface QuickSeedCmsPagesDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

type CmsPageTemplate = Omit<NewCmsPage, "id" | "createdAt" | "updatedAt">;
...
export function QuickSeedCmsPagesDialog({
  open,
  onOpenChange,
  onSuccess,
}: QuickSeedCmsPagesDialogProps) {
  const [items, setItems] = useState<(CmsPageTemplate & { tempId: string })[]>([]);
  ...
  useEffect(() => {
    ...
    const initial = DEFAULT_CMS_PAGES.map((page, index) => ({
      ...page,
  ...
  const seedMutation = trpc.administrator.cms.seedPages.useMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await seedMutation.mutateAsync({ customPages: payload });
      toast.success(res.message || "Đổ dữ liệu mẫu thành công!");
      utils.administrator.cms.getAll.invalidate();
      ...
      <DialogContent className="sm:max-w-[550px] w-[95vw]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            Đổ dữ liệu mẫu trang CMS
          </DialogTitle>
          <DialogDescription className="text-left mt-1 text-[13px]">
            Sắp xếp thứ tự của các trang CMS mẫu trước khi gieo vào hệ thống.
          </DialogDescription>
        </DialogHeader>

  ...
  return (
    ...
    >
      {isDraggable && (
        ...
        >
          <Icon icon="solar:sort-vertical-line-duotone" className="text-base" />
        </button>
      )}
      <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center shrink-0">
        <Icon icon={cat.icon || "solar:folder-line-duotone"} className="text-lg text-muted-foreground" />
      </div>
...
export default function AdminCategories() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: "Tổng chuyên mục", value: total, icon: "solar:folder-line-duotone", bgColor: "bg-blue-500/10", textColor: "text-blue-500" },
      ...
      { label: "Có icon", value: withIcon, icon: "solar:pallete-2-line-duotone", bgColor: "bg-violet-500/10", textColor: "text-violet-500" },
      { label: "Có mô tả", value: withDesc, icon: "solar:document-text-line-duotone", bgColor: "bg-amber-500/10", textColor: "text-amber-500" },
    ];
  }, [categories]);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
              <Icon icon="solar:folder-line-duotone" className="text-xl text-primary" />
            </div>
            <div>
      ...
      <AppDashed noTopBorder padding="p-0" scrollable>
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            <Icon icon="solar:folder-line-duotone" className="text-4xl text-muted-foreground/30" />
            ...
            <div className="flex items-center justify-between px-4 py-2 border-t border-border">
              ...
              {reordering && (
                <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                  <Icon icon="svg-spinners:ring-resize" className="text-xs animate-spin" />
                  Đang lưu...
                </span>
              )}
            </div>

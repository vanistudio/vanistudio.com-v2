...
interface QuickReorderProjectsDialogProps {
  open: boolean;
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="size-8 rounded-lg border border-border bg-muted/40 overflow-hidden flex items-center justify-center shrink-0">
          {thumbnail ? (
            ...
            <Icon icon="solar:folder-open-line-duotone" className="size-4 text-muted-foreground" />
          )}
        </div>
...
export function QuickReorderProjectsDialog({
  open,
  onOpenChange,
  onSuccess,
}: QuickReorderProjectsDialogProps) {
  const [items, setItems] = useState<Project[]>([]);
  const utils = trpc.useUtils();
  const projectsQuery = trpc.administrator.projects.getAll.useQuery(
    undefined,
    { enabled: open }
  );
  ...
  const reorderMutation = trpc.administrator.projects.reorderProjects.useMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Đã cập nhật thứ tự hiển thị dự án!");
      utils.administrator.projects.getAll.invalidate();
      ...
      <DialogContent className="sm:max-w-[550px] w-[95vw]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            Sắp xếp thứ tự dự án
          </DialogTitle>
          <DialogDescription className="text-left mt-1 text-[13px]">
            Kéo thả để sắp xếp thứ tự hiển thị của các Dự án trên trang công khai.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="overflow-y-auto min-h-[150px] max-h-[45vh] custom-scrollbar pr-1 flex flex-col gap-2">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center text-muted-foreground">
                <Icon icon="solar:folder-open-line-duotone" className="size-10 opacity-30 mb-2" />
                <span className="text-[13px]">Chưa có dự án nào.</span>
              </div>
            ) : (

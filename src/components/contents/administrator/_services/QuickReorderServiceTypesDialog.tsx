...
interface QuickReorderServiceTypesDialogProps {
  open: boolean;
...
export function QuickReorderServiceTypesDialog({
  open,
  onOpenChange,
  onSuccess,
}: QuickReorderServiceTypesDialogProps) {
  const [items, setItems] = useState<ServiceType[]>([]);
  const utils = trpc.useUtils();
  const typesQuery = trpc.administrator.services.getTypes.useQuery(
    undefined,
    { enabled: open }
  );
  ...
  const reorderMutation = trpc.administrator.services.reorderTypes.useMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Đã cập nhật thứ tự phân loại!");
      ...
      <DialogContent className="sm:max-w-[550px] w-[95vw]">
        <DialogHeader>
          ...
          <DialogDescription className="text-left mt-1 text-[13px]">
            Kéo thả để sắp xếp vị trí hiển thị của các Phân loại dịch vụ.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="overflow-y-auto min-h-[150px] max-h-[45vh] custom-scrollbar pr-1 flex flex-col gap-2">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center text-muted-foreground">
                ...
                <span className="text-[13px]">Chưa có dữ liệu nào trực thuộc.</span>
              </div>
            ) : (

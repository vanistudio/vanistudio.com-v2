...
import { DEFAULT_SERVICES, type DefaultService } from "@/defaults/services.default";
...
interface QuickSeedServicesDialogProps {
  open: boolean;
...
function SortableItem({
  ...
  const formattedPrice = useMemo(() => {
    ...
    const priceStr = basePrice.toLocaleString("vi-VN") + " đ";
    return priceType === "starting_at" ? `Từ ${priceStr}` : priceStr;
  }, [basePrice, priceType]);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <Badge variant="secondary" className="text-[9px] px-1 py-0 font-semibold uppercase">
              {typeName}
            </Badge>
      ...
      <Badge variant="outline" className="text-[10px] font-bold font-mono text-foreground border-border/80 bg-background/50 shrink-0">
        {formattedPrice}
      </Badge>
    </div>
  );
...
export function QuickSeedServicesDialog({
  open,
  onOpenChange,
  onSuccess,
}: QuickSeedServicesDialogProps) {
  const [items, setItems] = useState<(DefaultService & { tempId: string })[]>([]);
  ...
  useEffect(() => {
    ...
    const initial = DEFAULT_SERVICES.map((service, index) => ({
      ...service,
  ...
  const seedMutation = trpc.administrator.services.seedServices.useMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(res.message || "Đổ dữ liệu mẫu dịch vụ thành công!");
      ...
      <DialogContent className="sm:max-w-[580px] w-[95vw]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            Đổ dữ liệu mẫu dịch vụ
          </DialogTitle>
          <DialogDescription className="text-left mt-1 text-[13px]">
            Sắp xếp vị trí/thứ tự hiển thị của các dịch vụ mẫu trước khi đổ vào hệ thống.
          </DialogDescription>
        </DialogHeader>

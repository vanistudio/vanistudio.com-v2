...
export default function ToolCheckId() {
  ...
  const [result, setResult] = useState<{ uid: string; name?: string; avatar?: string; link?: string } | null>(null);
  ...
  const extractId = async () => {
    ...
    try {
      // Try to extract UID from URL patterns
      ...
      // If it's a pure numeric ID, use directly
      if (/^\d+$/.test(target)) {
        setResult({
          uid: target,
            ...
            <div className="flex flex-col gap-3 p-4 rounded-xl border border-border bg-muted/20">
              ...
              {result.link && (
                <a
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:bg-muted-background transition-colors"
                >

...
export const authRoutes = new Elysia({ prefix: "/api/auth" })
  ...
  .use(configProxy)
  .get("/github", ({ redirect }) => {
    const url = authenticationService.getGithubAuthUrl();
    return redirect(url);
  })
  .get("/github/callback", async ({ query, cookie, redirect, needsSetup }) => {
    try {
      ...
      const { token, needsOnboarding } = await authenticationService.handleGithubCallback(code);
  ...
  .get("/google", ({ redirect }) => {
    const url = authenticationService.getGoogleAuthUrl();
    return redirect(url);
  })
  .get("/google/callback", async ({ query, cookie, redirect, needsSetup }) => {
    try {
      ...
      const { token, needsOnboarding } = await authenticationService.handleGoogleCallback(code);
      cookie[COOKIE_NAME].set({ value: token, ...COOKIE_OPTIONS });
  ...
  .post("/onboarding", async ({ body, cookie, auth }) => {
    ...
    try {
      const { user, token: newToken } = await authenticationService.completeOnboarding(auth.userId, body);
  ...
  .post("/register", async ({ body, cookie }) => {
    try {
      ...
      const { token } = await authenticationService.register({ username, fullName, email, phone: phone || "", password });
      cookie[COOKIE_NAME].set({ value: token, ...COOKIE_OPTIONS });
      return { success: true };
    } catch (error: any) {
      return { success: false, error: error.message };
  ...
  .post("/login", async ({ body, cookie }) => {
    try {
      ...
      const { token, needsOnboarding } = await authenticationService.login(email, password);
      cookie[COOKIE_NAME].set({ value: token, ...COOKIE_OPTIONS });
